...
import { KeyValueData, KVInstance } from "./keyval";
...
export function loadSigner(iID: string) {
    try {
        const r = Handler.checkRoster() || Handler.checkDarc();
        if (r !== "") {
            Handler.prependLog(r);
            return;
        }
...
export function addRule(rID: string) {
    try {
        const r = Handler.checkRoster() || Handler.checkDarc() || Handler.checkSigner();
        if (r !== "") {
            Handler.prependLog(r);
            return;
        }
...
export function addKVpair(instIDID: string, keyID: string, valueID: string) {
    try {
        ...
        const valueStr = valueHolder.value;
        if (valueStr === "") {
            const c = confirm("This action will remove the key '" + keyStr + ". Are you sure?");
            if (!c) {
                return;
            }
        }
...
class Handler {
    ...
    static prependLog(...nodes: Array<Node | string>) {
    ...
    SetSigner(sid: Buffer) {
        ...
        try {
            const signer = Cothority.darc.SignerEd25519.fromBytes(sid);
            Handler.signer = signer;
        } catch (e) {
            Handler.prependLog("failed to create signer: " + e);
        }
        Handler.prependLog("signer '" + Handler.signer.toString() + "' set");
    }
            ...
            (r) => {
                ...
                r.getProofFromLatest(instIDStr).then(
                    (proof) => {
                        ...
                        const kvInstance = KeyValueData.decode(proof.value);
    ...
    AddKVpair(instID: Buffer, keyStr: string, valueStr: string) {
        ...
        rpc.then(
            (r) => {
                ...
                let buffValue = EMPTY_BUFFER;
                if (valueStr !== "") {
                    buffValue = Buffer.from(valueStr);
                }
